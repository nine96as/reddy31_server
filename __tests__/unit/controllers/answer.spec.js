...
describe('answer controller', () => {
  ...
  describe('showByQuestion', () => {
    ...
    it('should handle errors', async () => {
      ...
      jest.spyOn(Answer, 'find').mockRejectedValue(new Error('Error fetching answers'));

      await answerController.showByQuestion(mockReq, mockRes);

      expect(Answer.find).toHaveBeenCalledWith({ questionId: '123' });
      expect(mockStatus).toHaveBeenCalledWith(500);
      expect(mockJson).toHaveBeenCalledWith({ error: 'Error fetching answers' });
    });
  });
  ...
  describe('create', () => {
    ...
    it('should handle errors during answer creation', async () => {
      ...
      jest.spyOn(Answer.prototype, 'save').mockRejectedValue(new Error('Error creating answer'));

      await answerController.create(mockReq, mockRes);

      expect(Answer).toHaveBeenCalledWith(newAnswer);
      expect(mockStatus).toHaveBeenCalledWith(500);
